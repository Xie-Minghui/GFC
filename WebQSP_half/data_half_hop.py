...
class DataLoader(torch.utils.data.DataLoader):
    def __init__(self, input_dir, fn, bert_name, ent2id, rel2id, batch_size, training=False):
        ...
        for line in open(os.path.join(input_dir, 'fbwq_half/train.txt')):
            l = line.strip().split('\t')
            s = l[0].strip()
            p = l[1].strip()
            o = l[2].strip()
...
def load_data(input_dir, bert_name, batch_size):
    cache_fn = os.path.join(input_dir, 'processed_half_hop_fixed.pt')
    if os.path.exists(cache_fn):
        ...
        print('Read data...')
        ent2id = {}
        for line in open(os.path.join(input_dir, 'fbwq_half/entities.dict')):
            l = line.strip().split('\t')
            ent2id[l[0].strip()] = len(ent2id)
        rel2id = {}
        for line in open(os.path.join(input_dir, 'fbwq_half/relations.dict')):
            l = line.strip().split('\t')
